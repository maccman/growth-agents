...
12. **Environment variables** — Check if a `.env` file exists in the repo root. If not, copy `.env.example` to `.env`, then ask the user which AI providers they want to use, collect the API keys, and update the `.env` file.
...
Once everything is installed, run a quick smoke test: `pnpm start scripts/say_hello.ts`. If it succeeds, congratulate the user — they're all set to run marketing automation agents!
